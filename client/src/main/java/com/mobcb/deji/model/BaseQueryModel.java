...
public class BaseQueryModel implements Serializable {
    ...
    private Integer status =1; //状态 1正常，-1删除
    ...
    public static boolean isNotEmptyOrNull(String string) {
        string = emptyToNullAndTrim(string);
        if (null != string) {
            return true;
        }
        return false;
     ...
     * @return
     */
    public static String emptyToNullAndTrim(String string) {
        if (null == string) {
            return string;
        } else if (null != string && string.trim().length() == 0) {
            return null;
        }
        return string.trim();
    }
